...
y_train = np.log(X_train['loss'].values + shift)
...
def get_oof(clf):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(classes, classes)):
        ...
        for j in range(nbags):
            ...
            pred += np.exp(clf.predict(x_te)) - shift
            pred_test += np.exp(clf.predict(X_test)) - shift

        pred /= nbags
        pred_oob[test_index] = pred
        score = mean_absolute_error(np.exp(y_te) - shift, pred)
        print('Fold ', i, '- MAE:', score)

    return pred_oob, pred_test

svr = LinearSVR(C=0.021544346900318832)
# svr = Ridge()

svr_oof_train, svr_oof_test = get_oof(svr)

print("SVR-CV: {}".format(mean_absolute_error(np.exp(y_train) - shift, svr_oof_train)))

oof_train = pd.DataFrame({'id': X_train_id, 'loss': svr_oof_train})
oof_train.to_csv('oof2/svr_train_log.csv', index=False)

svr_oof_test /= n_folds

oof_test = pd.DataFrame({'id': X_test_id, 'loss': svr_oof_test})
oof_test.to_csv('oof2/svr_test_log.csv', index=False)
