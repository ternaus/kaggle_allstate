...
for column in list(train.select_dtypes(include=['object']).columns):
    if train[column].nunique() != test[column].nunique():
        ...
        tr_te[column] = tr_te[column].apply(lambda x: filter_cat(x), 1)
...
for (inTr, inTe) in folds:
    ...
    for j in range(nbags):
        ...
        pred_test += np.exp(
            model.predict_generator(generator=batch_generatorp(xtest, 800, False), val_samples=xtest.shape[0])[:, 0]) - 200
    pred /= nbags
    pred_oob[inTe] = pred
